...
public class Gnuplot {
    ...
    public String getGnuplotString() {
        ...
        try {
            buf.append("set terminal ").append(fig ? "fig" : "postscript eps").append(bw ? "" : " color").append("\n");
            buf.append("set output '").append(outputfile).append(fig ? ".fig" : ".eps").append("\n");
            if (title != null)
                buf.append("set title '").append(title).append("'\n");
            if (xlabel != null)
                buf.append("set xlabel '").append(xlabel).append("\n;");
            if (ylabel != null)
                buf.append("set ylabel '").append(ylabel).append("'\n");
            StringBuilder s = new StringBuilder("plot ");
            if (minx != maxx)
                s.append("[").append(minx).append(":").append(maxx).append("]");
            if (miny != maxy) {
                if (minx == maxx)
                    s.append("[]");
                s.append("[").append(miny).append(":").append(maxy).append("]");
            }
            s.append(" ");
            for (Object o : sources) {
                ...
                s.append(d);
                if (t != null)
                    s.append(" title '").append(t).append("'");
                s.append(",");
            }
    ...
    public StringBuffer addLaTeX(StringBuffer latex) {
        ...
        latex.append("\\includegraphics[scale=0.9]{").append(outputfile).append("}\n");
