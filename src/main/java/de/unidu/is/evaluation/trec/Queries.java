...
public class Queries {
	 ...
	 * 
	 * @param terms
	 *                   query terms, separated by spaces
	 * @return list of tuples (0: term, 1: tf)
	 */
	 ...
	 *                   query id
	 * @param attName
	 *                   attribute (schema element or alias) name
	 * @param operator
	 *                   operator name
	 * @return query
	 */
	...
	public static XIRQLQuery getXIRQLQuery(String queryID, String postfix,
			String attName, String operator) {
		StringBuffer xirql = new StringBuffer();
		List terms = getQueryTerms(queryID);
		double dl = 0;
		for (Iterator it = terms.iterator(); it.hasNext();) {
			Tuple tuple = (Tuple) it.next();
