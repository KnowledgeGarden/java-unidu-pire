...
public class Lemur {
	 ...
	 *                   base directory
	 * @param useSample
	 *                   if true, the sample will be used for the CORI index
	 */
	...
	public File createTextFile(String coll, Map termMap) {
		...
		try {
			...
			for (Iterator it = termMap.keySet().iterator(); it.hasNext();) {
	 ...
	 *                   collection name
	 * @param doLog
	 *                   if true, the output of the Lemur program is logged
	 */
	 ...
	 *                   if true, appends to that file
	 */
	public void createTopicFile(File topicFile, Query query, boolean append) {
		KeywordQuery keywordQuery = (KeywordQuery) query;
		PrintWriter pw = null;
		try {
			pw = new PrintWriter(new FileWriter(topicFile, append));
			String qID = query.getQueryID();
			int h = qID.indexOf("_");
			if (h != -1)
				qID = qID.substring(0, h);
			pw.println("<DOC " + qID + ">");
			for (Iterator iter = keywordQuery.getKeywords().iterator(); iter
					.hasNext();) {
	 ...
	 *                   collection name
	 * @param topicsName
	 *                   name of the topics (for the topic and the result file)
	 * @param mode
	 *                   Lemur mode
	 * @param numDocs
	 *                   number of documents
	 ...
	 * 
	 * @param colls
	 *                   collections to be used for the index
	 * @param doLog
	 *                   if true, the output of the Lemur program is logged
	 */
	public void indexCORI(String[] colls, boolean doLog) {
		...
		try {
			...
			for (int i = 0; i < colls.length; i++)
				pw.println(getSampleFile(colls[i]));
			pw.close();
			pw = null;
	 ...
	 *                   Lemur arguments
	 * @param doLog
	 *                   if true, logs the output
	 */
	private void callProgram(String[] args, boolean doLog) throws IOException {
