...
public class HyREXSchema extends Schema {
	 ...
	 */
	private static PropertyMap defaultOperators;
	...
	static {
		...
		defaultOperators.put(NameDT.NAME, NameDT.SOUNDEX);
		...
		try {
			...
			for (Node node; (node = it.nextNode()) != null;) {
				...
				for (Node n; (n = it2.nextNode()) != null;) {
					...
					if (!xpath.endsWith("/text()") && xpath.indexOf("@") == -1)
						xpath += "/text()";
					if (!useXPathForQuery)
	...
	public void addDefaultOperators() {
		for (Iterator iter = getXPaths().iterator(); iter.hasNext();) {
