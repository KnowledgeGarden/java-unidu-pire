...
public class DelegatedPropertyMap extends PropertyMap {
     ...
     *
     * @param init map for initialisation (may be null)
     */
    public void init(Map init) {
        if (init != null)
            for (Object o : init.keySet()) {
                String key = (String) o;
     ...
     * @param key         key
     * @param value       value for the key
     * @param callChanged if true, calls <code>changed()</code>
     * @return previous value
     */
     ...
     *
     * @param map map to which calls are delegated
     */
