...
public class DB {
	...
	public DB(String uri, String driver, String user, String password,
		...
		this.dbUser = user;
		this.logger = logger;
		result2conn = Collections.synchronizedMap(new HashMap());
		result2stmt = Collections.synchronizedMap(new HashMap());
		...
		formatter = SQLFormatterFactory.newFactory().newFormatter(this);
	}
	 ...
	 * @throws SQLException
	 */
	private DataSource getDataSource(String uri, String driver, String user,
			String password) throws SQLException, IOException {
		DataSource dataSource = (DataSource) dataSources.get(uri);
		if (dataSource == null) {
	 ...
	 *                   SQL query
	 * @param info
	 *                   flag indicating whether logging as info is wanted
	 * @return result set
	 */
	 ...
	 * @param fieldToCheck
	 *                   the field to be checked first. The corresponding value is
	 ...
	 *                   the elements to be inserted
	 * @param fieldsToCheck
	 *                   the fields to be checked first. The corresponding values are
	...
	public int insertIfMissing(String table, Map elements,
		...
		try {
			if (table != null && !table.equals("") && fieldsToCheck != null
	 ...
	 *                   the table name
	 * @param elements
	 *                   the elements - the key of the map is the field name, the value
	 *                   its value.
	...
	public int insert(String table, Map elements, boolean info) {
		...
		int ret = -1;
		String sql = null;
		if (elements != null && table != null && !table.equals("")) {
			try {
				String fields = ""; // all field names
				String parameters = ""; // all "?" parameters for
				// prep. stmt.
				boolean firstElem = true;
				for (Iterator e = (elements.keySet()).iterator(); e.hasNext();) {
					if (!firstElem) {
						fields += ",";
						parameters += ",";
					} else
						firstElem = false;
					fields += (String) e.next();
					parameters += "?";
				}
	...
	public int remove(String table, Map removeCondition, boolean info) {
		...
		try {
			if (table != null && !table.equals("")) {
				String whereClause = null;
				Vector values = null;
				for (Iterator e = removeCondition.keySet().iterator(); e
						.hasNext();) {
