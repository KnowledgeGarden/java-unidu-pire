...
public class RelationBase {
     ...
     */
    private final PDatalogSQLFormatter formatter;
    /**
     * PDatalog++ configuration.
     */
    protected final PropertyMap config;
    /**
     * Logger for rules.
     */
    protected final Logger rulesLogger = Log.getLogger("unidu.pdatalog.rules");
    /**
     * Logger for facts.
     */
    protected final Logger factsLogger = Log.getLogger("unidu.pdatalog.facts");
    ...
    public RelationBase(DB db) {
        ...
        for (Object o : config.getAll("pdatalog.edbcomputed")) {
            String relationClassName = (String) o;
            try {
     ...
     *
     * @param list     list containing arguments (
    ...
    private SQL transform(Rule rule) {
        ...
        for (Object item : binding.keySet()) {
        ...
        if (probOperator != null) { // use mapping function
            if (probOperator instanceof FunctionExpression) {
                ...
                if (op.getName().equals("groupsum")) {
                    ...
                    // in head
                    Expression exp = op.getArgument();
                    probOperator = new FunctionExpression("sum", exp);
                }
            }
            Map args = new HashMap(binding);
     ...
     *
     * @param relation IDB relation
     * @param rule     rule to be evaluated
     */
     ...
     * @param relation IDB relation
     * @param rule     rule to be evaluated
     * @param addIndex if true, an DB index is added
     */
     ...
     *
     * @param relation    target relation
     * @param source      temporary source relation
     * @param target      temporary target relation
     * @param createIndex if true, a DB index is created
     */
    ...
    public int getTupleCount(Relation rel) {
        ...
        ResultSet rs = null;
        try {
            StringBuilder s = new StringBuilder("prob");
            for (int i = 0; i < rel.getArity(); i++)
                s = new StringBuilder("concat(" + s + ",concat('@@@',arg" + i + "))");
