...
public class Rule {
    ...
    public static Expression getMapping(double prob) {
        return new ProductExpression(
                new PlainExpression(String.valueOf(prob)),
                new ProbExpression());
    }
    ...
    public Expression[] getAllArgumentsAsArray() {
        ...
        return (Expression[]) set.toArray(new Expression[0]);
    }
    ...
    public String toString() {
        StringBuilder ret = new StringBuilder((head != null ? head.toString() : ""));
        if (literalCount() > 0)
            ret.append(" :- ");
        for (int i = 0; i < literalCount(); i++)
            ret.append(literalAt(i)).append(i < literalCount() - 1 ? " & " : "");
        return ret + " | " + mapping + ".";
    }
