...
import de.unidu.is.util.StreamPropertyMap;
...
public class SQLFormatterImplementation implements SQLFormatter {
	 ...
	 * @param types
	 *                   types of the columns, can be subject to substitution
	 */
	 ...
	 *                   name of the result table (if <code>null</code>, the insert
	...
	public String getSelect(SQL sql) {
		...
		if (sql.getFrom() != null && sql.getFrom().size() > 0) {
			...
			if (sql.getSelect() != null)
				for (Iterator it = sql.getSelect().iterator(); it.hasNext();) {
			...
			if (sql.getWhere() != null && sql.getWhere().size() > 0) {
				...
				for (Iterator it = sql.getWhere().iterator(); it.hasNext();) {
					Expression arg = (Expression) it.next();
	 ...
	 *                   imvolved columns
	 * @param textCols
	 *                   flag indicating that the column is a text column
	 */
